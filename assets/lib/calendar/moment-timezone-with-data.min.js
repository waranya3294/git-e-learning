!(function (M, z) {
  ...
  function O(M) {
    var z = 0,
      ...
      c = 0,
      q = 1;
    for (45 === M.charCodeAt(0) && (q = -(z = 1)); z < p.length; z++)
      c = 60 * c + R(p.charCodeAt(z));
    for (z = 0; z < O.length; z++) (A /= 60), (c += R(O.charCodeAt(z)) * A);
    return c * q;
  }
  function a(M) {
  ...
  function N(M, z) {
    for (var b, p; (p = 6e4 * (((z.at - M.at) / 12e4) | 0)); )
      (b = new X(new Date(M.at + p))).offset === M.offset ? (M = b) : (z = b);
    return M;
  }
  function u(M, z) {
    ...
    var b,
      ...
      A = (function () {
        var M,
          ...
          O = new X(new Date(p, 0, 1)),
          A = [O];
        for (b = 1; b < 48; b++)
          (z = new X(new Date(p, b, 1))).offset !== O.offset &&
            ((M = N(O, z)), A.push(M), A.push(new X(new Date(M.at + 6e4)))),
            (O = z);
        for (b = 0; b < 4; b++)
          A.push(new X(new Date(p + b, 0, 1))),
            A.push(new X(new Date(p + b, 6, 1)));
        return A;
      })(),
      c = A.length,
      q = (function (M) {
        var z,
          ...
          c = [];
        for (z = 0; z < O; z++)
          for (b in (p = d[M[z].offset] || {}))
            p.hasOwnProperty(b) && (A[b] = !0);
        for (z in A) A.hasOwnProperty(z) && c.push(W[z]);
        return c;
      })(A),
      o = [];
    for (p = 0; p < q.length; p++) {
      for (b = new i(l(q[p]), c), O = 0; O < c; O++) b.scoreOffsetAt(A[O]);
      o.push(b);
    }
    return o.sort(u), 0 < o.length ? o[0].zone.name : void 0;
  }
  function T(M) {
  ...
  function l(M, z) {
    ...
    return p instanceof f
      ...
      ? ((p = new f(p)), (c[M] = p))
      : q[M] && z !== l && (b = l(q[M], l))
      ? ((p = c[M] = new f())._set(b), (p.name = W[M]), p)
      : null;
  }
  function s(M) {
    var z, b, p, O;
  ...
  function S(M) {
    var z = Array.prototype.slice.call(arguments, 0, -1),
      ...
      p = l(b),
      O = A.utc.apply(null, z);
    return (
      p && !A.isMoment(M) && E(O) && O.add(p.parse(O), "minutes"), O.tz(b), O
    );
  }
  (b < 2 || (2 == b && p < 6)) &&
    C(
    ...
    (S.names = function () {
      var M,
        z = [];
      for (M in W)
        W.hasOwnProperty(M) && (c[M] || c[q[M]]) && W[M] && z.push(W[M]);
      return z.sort();
    }),
    (S.Zone = f),
    (S.unpack = n),
